...
<div class="container text-end">
    ...
    <!-- Add Product Button -->
    <a href="{{ route('store.products.create') }}" class="btn btn-primary btn-rounded btn-fw" style="margin: 10px">
        إضافة منتج جديد
    </a>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    <form action="{{ route('store.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Are you sure?');" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-rounded btn-sm"><i class="fa fa-trash"></i></button>
                    </form>
                    <a href="{{ route('store.products.edit', $product->id) }}" class="btn btn-warning btn-rounded btn-sm">
                        <i class="fa fa-edit"></i>
                    </a>
                    <a href="{{ route('store.products.show', $product->id) }}" class="btn btn-info btn-rounded btn-sm" style="display:inline-block;">
                        <i class="fa fa-eye"></i>
                    </a>
                </td>
                <td>{{ $product->price }}</td>
